...
void execution( int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	...
	//Step 2.1
	char packets[500];
	printf("Hoeveel packets heb je nodig?");
	scanf("%s",packets);
	int packet_number= atoi(packets);
	int number_of_bytes_send = 0;
	number_of_bytes_send = sendto( internet_socket,packets, strlen(packets),  0, internet_address, internet_address_length );
	...
	//Step 2.2
	int aantal_verstuurd= 0;
	int packet_num= atoi(packets);
